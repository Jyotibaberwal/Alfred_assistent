...
def get_gemini_parsed_command(user_query):
    try:
        gemini_prompt = (
            f"Interpret the following user's request and respond with the exact command keyword or action only. "
            f"Do not add any extra sentences. Examples of actions include 'schedule_appointment', "
            f"'show appointments', 'send email', 'search web', 'play music', 'calculate','perform calculation','open_e_commerce',etc.  "
            f"Request: '{user_query}'"
        )
        response = chat.send_message(gemini_prompt, stream=False)
        parsed_command = response.text.strip().lower()
        print(f"Gemini Parsed Command: {parsed_command}")
...
def process_query(query):
    # Parse the user input via Gemini to identify the action
    parsed_command = get_gemini_parsed_command(query)
    if 'schedule_appointment' in parsed_command:
        schedule_appointment()
    
    elif 'show appointments' in parsed_command:
        show_appointments()

    elif 'send email' in parsed_command:
        send_email()

    elif 'read email' in parsed_command:
        read_emails()

    elif 'add to do' in parsed_command:
        add_to_do()

    elif 'show todo' in parsed_command:
        show_todo_list()

    elif 'make call' in parsed_command or 'call' in parsed_command:
        make_call()

    elif 'send message' in parsed_command or 'message' in parsed_command:
        send_message()

    elif 'take note' in parsed_command:
        take_note()

    elif 'show note' in parsed_command or 'show notes' in parsed_command:
        show_note()

    elif 'search' in parsed_command and 'web' in parsed_command:
        search_query = query.replace('search', '').replace('on the web', '').strip()
        search_web(search_query)

    elif 'calculate' in parsed_command:
        perform_calculation()

    elif 'weather update' in parsed_command:
        location = takeCommand("Which location's weather report would you like?")
        if location != "none":
            get_weather_report(location)

    elif 'play music' in parsed_command:
        play_music()

    elif 'stop music' in parsed_command:
        stop_music()

    elif 'news update' in parsed_command:
        get_news_updates()

    elif 'restaurant reservation' in parsed_command:
        make_restaurant_reservation()

    elif 'translate' in parsed_command:
        translate_text()

    elif 'recipe' in parsed_command or 'suggest recipe' in parsed_command or 'what should I cook' in parsed_command:
        get_recipe_suggestion()

    elif 'podcast' in parsed_command:
        play_podcast()

    elif 'video' in parsed_command:
        play_video()
    
    elif 'social media' in parsed_command:
        open_social_media()

    elif 'productivity' in parsed_command:
        open_productivity_suite()

    elif 'shop' in parsed_command:
        open_e_commerce()

    elif 'exit' in parsed_command or 'stop' in parsed_command:
        speak("Goodbye!")
        return False
    
    elif parsed_command == "none":
        speak("I didn't understand the request. Can you please try again?")
